...
{
protected:
	std::unordered_map<PType, std::function<std::shared_ptr<Base>()>> creator;
	std::unordered_map<PType, std::vector<std::shared_ptr<Base>>> products;
	int size = 10;
...
public:
	...
	std::shared_ptr<Base> createObject(PType type);
};

template<typename T>
void Factory::registerType(PType type)
{
	creator[type] = []() { return std::make_shared<T>(); };
	products[type] = std::vector<std::shared_ptr<Base>>();
}

std::shared_ptr<Base> Factory::createObject(PType type)
{
	auto it = creator.find(type);
	if (it == creator.end()) return std::shared_ptr<Base>();

	auto newObj = it->second();
	products[type].push_back(newObj);

	return products[type].back();
}

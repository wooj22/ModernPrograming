...
// 1.괄호로 둘러쌓이지 않은, '식별자 표현식(id-expression)' 이라면, 
	해당 식의 타입을 얻을 수 있습니다.		***
...
// 2.식별자 표현식이 '아닌' 식을 전달하면,
	1) 만일 식의 값 종류가 prvalue 라면	decltype 는 T 가 됩니다.		***
	   p rvalue : 임시객체, 리터럴, 산술연산의 결과 등

	   int a, b;
	   decltype(a + b) c;		// c 의 타입은?  //int

	2) 만일 식의 값 종류가 lvalue 라면	decltype 는 T& 가 됩니다.	***
	l value : 이름이 있는 모든 변수, 배열, 함수 호출의 결과 등
	   int a;
	   decltype(a) b;			// b 의 타입은??  //int

	   int a;
	   decltype((a)) b;			// b 의 타입은??  //int&  //()를 하면 lvalue

	3) 만일 식의 값 종류가 xvalue 라면	decltype 는 T&& 가 됩니다.	***
